  ...
  <div style="padding-bottom: 50px">
    ...
    >
      <span class="text-bold" style="font-size: 1.2rem"
        >Birthday Greetings Notification</span
      >
...
const getNotification = async (x) => {
  ...
  const response = await api({
    method: 'GET',
    url: `/cms-service/notification/notif?type=Birthday`
    })
      ...
      if (response.ok) {
        ...
        savedNotificationData.value.notificationBefore =
          response.data.data.notification_before;
...
const dragFile = async (e) => {q-editorn
  ...
  if (editorRef.value.hasError || notificationBeforeRef.value.hasError) {
    ...
    data: {
        ...
        if (response.ok) {
          ...
          // var src = URL.createObjectURL(thefile.value);
          // var preview = document.getElementById("file-ip-1-preview");
          // preview.src = src;
          // preview.style.display = "block";
