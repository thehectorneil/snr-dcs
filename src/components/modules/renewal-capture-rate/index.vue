 ...
 */
 const getParams = () => {
  ...
  const params = {
    ...
    expiration_date: filters.expDate,
    renewed: filters.renewed,
 ...
 */
 const headerAction = async (params) => {
  ...
  const response = await api({
    method: 'GET',
    url: '/cms-service/dcs-mangement/reports?renewal_capture_download',
    loading: true,
    param: getParams()
  })
  if(!response.ok) {
  ...
  downloadCsv(response.data, 'renewal_capture_rate')
}
