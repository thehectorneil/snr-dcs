...
<div class="col-12 col-sm-6 col-md-6 col-lg-6 text-right">
  ...
  <BaseModal
    ...
    <template v-slot:body>
      ...
      <div class="collumn">
        <BaseLabel title="Registration Status" />
        <BaseSelect
          v-model="form.regStatus"
          :data="regStatusList"/>
      </div>
      <div class="collumn">
        <BaseLabel title="Volume Shopper" />
        <BaseSelect
          v-model="form.volumeShopper"
...
const regStatusList = ref(['All', 'Linked', 'Pending', 'Timed Out'])
const volumeShopperList = ref(['All', 'Yes', 'No'])
const cardStatusList = ref(['All', 'Gold', 'Business'])
/**
...
const resetFilter = () => {
  ...
  form.value.regStatus = 'All'
  form.value.volumeShopper = 'All'
  form.value.cardType = 'All'
  form.value.expDate = ''
  emit('reload', form.value)
}
</script>